...
module.exports = class TemplateModel {
    ...
    async getTripleBannerItem(filter = []) {
        ...
        switch (filter['seo']) {
            case 'kupon-yagmuru':
                let couponManufacturerIds = await query('SELECT DISTINCT manufacturer_id FROM ys_coupon_to_manufacturer ctm LEFT JOIN ys_coupon c ON c.coupon_id=ctm.coupon_id WHERE c.date_end>NOW()');
                let couponCategoryIds = await query('SELECT DISTINCT category_id FROM ys_coupon_to_category ctc LEFT JOIN ys_coupon c ON c.coupon_id=ctc.coupon_id WHERE c.date_end>NOW()');
            ...
            case 'kargo-bedava':
                ...
                let freeShippingIds = await productModel.getProductFreeShipping(start, limit)
                let pids = ''
    ...
    async updateOneTripleBanner(data) {
        ...
        let sql = `UPDATE ys_triple_banner SET link='${data.link ? data.link : ''}',sm_banner='${data.sm_banner ? data.sm_banner : ''}', lg_banner='${data.lg_banner ? data.lg_banner : ''}' 
