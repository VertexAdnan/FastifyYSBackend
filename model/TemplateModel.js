...
module.exports = class TemplateModel {
  ...
  async extrasSpecials (filter = []) {
    let sql = `SELECT * FROM ys_extras_specials WHERE extra_id = ${
      filter['extra_id'] ? filter['extra_id'] : 1
    }`
  ...
  async updateSlider (data) {
    let sql = `UPDATE ys_sliders_temp SET title_id = ${
      data.title_id
    }, data = '${JSON.stringify(data.data)}' WHERE slider_id = ${
      data.slider_id
    }`
  ...
  async getTripleBannerItem (filter = []) {
    ...
    switch (filter['seo']) {
      case 'kupon-yagmuru':
        let couponManufacturerIds = '';

        let couponCategoryIds = '';

        if(filter['fc']){
          couponCategoryIds = filter['fc'];
        } else {
          couponCategoryIds = await query(
            `SELECT DISTINCT category_id FROM ys_coupon_to_category ctc LEFT JOIN ys_coupon c ON c.coupon_id=ctc.coupon_id WHERE c.date_end>NOW() LIMIT ${start},${limit}`
          )
        }

        if(filter['fm']){
          couponManufacturerIds = filter['fm'];
        } else {
          couponManufacturerIds = await query(
            `SELECT DISTINCT manufacturer_id FROM ys_coupon_to_manufacturer ctm LEFT JOIN ys_coupon c ON c.coupon_id=ctm.coupon_id WHERE c.date_end>NOW() LIMIT ${start},${limit}`
          )
        }
        ...
        if(typeof couponCategoryIds != 'string' && couponCategoryIds){
          couponCategoryIds.map(val => {
            cids += val.category_id + ','
          })
        } else {
          cids = couponCategoryIds;
        }

        if(typeof couponManufacturerIds != 'string' && couponManufacturerIds){
          couponManufacturerIds.map(val => {
            mids += val.manufacturer_id + ','
          })
        } else {
          mids += couponManufacturerIds
        }
  ...
  async updateOneTripleBanner (data) {
    let sql = `UPDATE ys_triple_banner SET link='${
      data.link ? data.link : ''
    }',sm_banner='${data.sm_banner ? data.sm_banner : ''}', lg_banner='${
      data.lg_banner ? data.lg_banner : ''
    }' 
